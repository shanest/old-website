...
//I'm not sure where this part of code should be
const queryString = window.location.search; //to extract queryString
const urlParams = new URLSearchParams(queryString); //parse URL paramteres
const PROLIFIC_PID = urlParams.get('PROLIFIC_PID') //get Prolific_ID
	...
	return slide({
		...
		present_handle: function(stim) {
		    ...
		    clearAll();
  			...
  			setTimeout(function(){
  				blank_screen(Date.now());
  			}, 200);


        function blank_screen(init_time) {
          $(document).unbind('keydown');
			    $(document).unbind('keyup');
        ...
        //  $(".display_condition").show();
          ...
          $(document).keyup(function(event) {
				    if(event.which == CHAR) {
					    _s.blank = Date.now() - init_time; // in milliseconds
             clearAll();
             setTimeout(function(){
       				display_one(Date.now());
       			}, 200);
				    } else {
					    $(".err").html("Press the K BUTTON to advance.");
					    $(".err").show();
				    }
			    });
        }
  ...
  slides.subj_info =  slide({
    ...
    $(".err").show();
		...
		fluent: $('input[name="fluent"]:checked').val(),
	        // prolific_id: $("#prolific_id").val(PROLIFIC_PID),

  ...
  slides.single_trial = slide({
    ...
    present_handle : function(stim) {
	    ...
	    function display_one() {
		    ...
		    $(document).keydown(function(event) {
			    if(event.which == 32) { // space bar = 32
				    _s.read_time_one = Date.now() - init_time; // in milliseconds
				    clearAll();
				    setTimeout(display_two, 500);
			    } else {
				    $(".err").html("Press the space bar to advance.");
				    $(".err").show();
			    }
		    });
	    }
	    ...
	    function display_three() {
		    ...
		    $(".true_response").html("Press 'z' for True.");
		    $(".true_response").show();
		    $(".false_response").html("Press '.' for False.");
		    $(".false_response").show();

		    init_time = Date.now();
		    $(document).keydown(function(event) {
			    if(event.which == 90 || event.which == 190 ) { // z = 90, . = 190
				    _s.decision_time = Date.now() - init_time; // in milliseconds
				    _s.response = event.which == 90 ? "True" : "False"; // TODO: depends on condition?
				    clearAll();
				    _s.log_responses();
			    }
		    });
